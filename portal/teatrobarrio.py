...
async def rq_to_page(r: ClientResponse):
    ...
    des = MD.convert(soup.select_one('section > div:first-child > div.elementor-col-33[data-id][data-element_type="column"]:last-child > div.elementor-widget-wrap.elementor-element-populated:has(p)'))
    return Page(
        duration=duration,
        url=root,
...
class TeatroBarrio(Base):
    ...
    def __item_to_event(self, i: Item):
        desc = i.description
        if i.summary is not None and (i.description is None or len(i.summary)>len(i.description)):
            desc = i.summary
        return Event(
            id="tb"+to_uuid(i.url),
            url=i.url,
    ...
    def __find_category(self, i: Item):
        if re_or(
            ...
            i.description,
            r"Para niñas y niños de \d meses",
            flags=re.I
        ):
